...
static void pci_pme_list_scan(struct work_struct *work)
{
	...
	if (!list_empty(&pci_pme_list))
		queue_delayed_work(system_freezable_power_efficient_wq, system_freezable_wq, &pci_pme_work,
	...
	if (dev->pme_poll) {
		...
		if (enable) {
			...
			if (list_is_singular(&pci_pme_list))
				queue_delayed_work(system_freezable_power_efficient_wq, system_freezable_wq,
						   &pci_pme_work,
						   msecs_to_jiffies(PME_TIMEOUT));
			mutex_unlock(&pci_pme_list_mutex);
		} else {
